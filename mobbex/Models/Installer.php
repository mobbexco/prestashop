...
class Installer
{
    ...
    public function createTables()
    {
        ...
        foreach (['mobbex_cache', 'customfields', 'task', 'transaction'] as $table) {
            $query = str_replace(
                ['DB_PREFIX_', 'ENGINE_TYPE'],
                [_DB_PREFIX_, _MYSQL_ENGINE_],
                file_get_contents(dirname(__FILE__) ."/../".($table === 'mobbex_cache' ? "vendor/mobbexco/php-plugins-sdk/src/" : ''). "sql/$table.sql")
            );
