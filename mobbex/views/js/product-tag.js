...
function displayProductTag() {
  ...
  if (!container) {
    console.warn("Mobbex Warning: container not found in product-tags.js", container);
    return;
  }

  const products = container.querySelectorAll(anchorProp.list);
  if (products.length === 0) {
    console.warn("Mobbex Warning: no products found with selector " + anchorProp.list + " in product-tags.js", products);
    return;
  }
  ...
  function addSourceFlag(product, eImg, plan) {
    if (product.querySelector('.mobbex-wrapper.mobbex-id-' + plan.id)) {
      console.log("Mobbex: Flag already exists for product " + plan.id + ". Skipping creation.");
      return;
    }
    ...
    + (plan.count < 9 ? "2" : "1.85") + "rem'>" + plan.count + "</span>"
  ...
  function addFinanceBanner(product, ePrice, plan) {
    if (product.querySelector('.mobbex-product-banner.mobbex-id-' + plan.id)) {
      console.log("Mobbex: Banner already exists for product " + plan.id + ". Skipping creation.");
      return;
    }
...
function initProductTags() {
  ...
  if (productListContainer) {
    ...
    console.error("Mobbex Error: document.body not found for MutationObserver. This should not happen.");
  }
}
