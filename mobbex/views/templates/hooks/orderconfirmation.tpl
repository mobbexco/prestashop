    ...
    <table class="table-response">
        ...
        <tr align="center">
            ...
            {foreach from=$transactions item=transaction key=key}
                {if $transaction->source_type eq "card"}

                    <tr align="left" style="border-top: 1px solid rgb(68, 68, 68);">
                        <td>{l s='Card' mod='mobbex'}</td>
                        <td>{$transaction->source_name|escape:'htmlall':'UTF-8'}</td>
                    </tr>
                    <tr align="left">
                        <td>{l s='Number' mod='mobbex'}</td>
                        <td>{$transaction->source_number|escape:'htmlall':'UTF-8'}</td>
                    </tr>
    
                        {if $transaction->installment_name}
                            <tr align="left" style="margin-bottom: 10px;">
                                <td>{l s='Selected Plan' mod='mobbex'}</td>
                                <td>{$transaction->installment_name|escape:'htmlall':'UTF-8'}</td>
                            </tr>
                        {/if}
                {/if}
    
                {if $transaction->source_type eq "cash"}
                    ...
                    <img src="{$transaction->source_url}" style="width: 100%; max-width: 450px; padding: 10px; display: block;" />
